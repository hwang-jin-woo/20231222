...
import { getMoviesNowPlaying, getMoviesPopular, getMoviesTopRated, getMoviesUpcoming, genreList, getBackDropUrl } from './api'
...
export function Products() {
  ...
  return <>
    ...
    {categoryList.map((cat,i)=> {
      if (i === +catIndex) {
        return <Btn key={i} onClick={()=>setCategory(i)} className='active'>{cat.name}</Btn>;
      } else {
        return <Btn key={i} onClick={()=>setCategory(i)}>{cat.name}</Btn>;
      }      
    })}
    <Container>
